/**
 * Seqvio 框架介绍 — 全中文 · 四幕 · 单笔尖串行绘制
 */

import React from 'react';
import { VideoComposition, Scene, Transition } from '@seqvio/core';
import {
  WhiteboardScene,
  DrawText,
  DrawShape,
  Hand,
  excalidrawTheme,
} from '@seqvio/whiteboard';

const W = 1280;
const H = 720;
const ACCENT = '#3498db';
const ACCENT2 = '#27ae60';
const MUTED = '#7f8c8d';
const CTA = '#e74c3c';

function TitleScene() {
  return (
    <WhiteboardScene width={W} height={H} texture="whiteboard" theme={excalidrawTheme}>
      <DrawText
        text="程序化视频"
        fontSize={16}
        fontWeight="bold"
        position={{ x: W / 2, y: 195 }}
        align="center"
        start={0}
        duration={24}
        strokeColor={ACCENT}
      />
      <DrawText
        text="统一视频框架"
        fontSize={52}
        fontWeight="bold"
        position={{ x: W / 2, y: 275 }}
        align="center"
        start={0}
        duration={32}
      />
      <DrawText
        text="结构化内容 · 手绘讲解 · 一键出片"
        fontSize={24}
        position={{ x: W / 2, y: 348 }}
        align="center"
        start={0}
        duration={26}
        strokeColor={MUTED}
      />
      <DrawShape
        type="rounded-rectangle"
        position={{ x: 300, y: 420 }}
        size={{ width: 100, height: 70 }}
        start={0}
        duration={16}
        fillColor="#ffffff"
      />
      <DrawText
        text="分镜"
        fontSize={15}
        fontWeight="bold"
        position={{ x: 350, y: 462 }}
        align="center"
        start={0}
        duration={12}
      />
      <DrawShape
        type="arrow"
        from={{ x: 410, y: 455 }}
        to={{ x: 490, y: 455 }}
        start={0}
        duration={12}
        strokeColor={ACCENT}
      />
      <DrawShape
        type="rounded-rectangle"
        position={{ x: 500, y: 420 }}
        size={{ width: 100, height: 70 }}
        start={0}
        duration={12}
        fillColor="#ffffff"
      />
      <DrawText
        text="渲染"
        fontSize={16}
        fontWeight="bold"
        position={{ x: 550, y: 462 }}
        align="center"
        start={0}
        duration={12}
      />
      <DrawShape
        type="arrow"
        from={{ x: 610, y: 455 }}
        to={{ x: 690, y: 455 }}
        start={0}
        duration={10}
        strokeColor={ACCENT2}
      />
      <DrawShape
        type="rounded-rectangle"
        position={{ x: 700, y: 408 }}
        size={{ width: 44, height: 84 }}
        start={0}
        duration={10}
        strokeColor={ACCENT2}
        fillColor="rgba(39,174,96,0.15)"
      />
      <DrawText
        text="视频"
        fontSize={14}
        fontWeight="bold"
        position={{ x: 722, y: 462 }}
        align="center"
        start={0}
        duration={12}
        strokeColor={ACCENT2}
      />
      <Hand action="write" follow={true} visible={true} />
    </WhiteboardScene>
  );
}

function PipelineScene() {
  const steps = ['编写分镜脚本', '打包与校验', '渲染成片'];
  return (
    <WhiteboardScene width={W} height={H} texture="whiteboard" theme={excalidrawTheme}>
      <DrawText
        text="核心流程"
        fontSize={20}
        fontWeight="bold"
        position={{ x: 200, y: 180 }}
        align="left"
        start={0}
        duration={28}
        strokeColor={ACCENT}
      />
      {steps.map((label, i) => (
        <React.Fragment key={label}>
          <DrawShape
            type="circle"
            position={{ x: 120, y: 250 + i * 70 }}
            size={44}
            start={0}
            duration={14}
            strokeColor={i === 2 ? ACCENT2 : ACCENT}
            fillColor={i === 2 ? ACCENT2 : ACCENT}
          />
          <DrawText
            text={String(i + 1)}
            fontSize={18}
            fontWeight="bold"
            position={{ x: 120, y: 258 + i * 70 }}
            align="center"
            start={0}
            duration={12}
            strokeColor="#ffffff"
            textRender="stroke"
          />
          <DrawText
            text={label}
            fontSize={26}
            fontWeight="bold"
            position={{ x: 180, y: 250 + i * 70 }}
            align="left"
            start={0}
            duration={18}
          />
        </React.Fragment>
      ))}
      <DrawShape
        type="rounded-rectangle"
        position={{ x: 720, y: 380 }}
        size={{ width: 90, height: 70 }}
        start={0}
        duration={14}
        fillColor="#ffffff"
      />
      <DrawShape
        type="arrow"
        from={{ x: 820, y: 415 }}
        to={{ x: 900, y: 415 }}
        start={0}
        duration={12}
        strokeColor={ACCENT}
      />
      <DrawShape
        type="rounded-rectangle"
        position={{ x: 910, y: 380 }}
        size={{ width: 90, height: 70 }}
        start={0}
        duration={12}
        fillColor="#ffffff"
      />
      <DrawShape
        type="arrow"
        from={{ x: 1010, y: 415 }}
        to={{ x: 1080, y: 415 }}
        start={0}
        duration={10}
        strokeColor={ACCENT2}
      />
      <DrawText
        text="成片"
        fontSize={14}
        fontWeight="bold"
        position={{ x: 1110, y: 422 }}
        align="center"
        start={0}
        duration={10}
        strokeColor={ACCENT2}
      />
      <Hand action="write" follow={true} visible={true} />
    </WhiteboardScene>
  );
}

function FeatureScene() {
  return (
    <WhiteboardScene width={W} height={H} texture="whiteboard" theme={excalidrawTheme}>
      <DrawShape
        type="rounded-rectangle"
        position={{ x: 240, y: 200 }}
        size={{ width: 800, height: 320 }}
        start={0}
        duration={22}
        strokeColor={ACCENT2}
      />
      <DrawText
        text="面向 AI 智能体"
        fontSize={40}
        fontWeight="bold"
        position={{ x: W / 2, y: 295 }}
        align="center"
        start={0}
        duration={22}
        strokeColor={ACCENT2}
      />
      <DrawText
        text="白板动画 · 手写分镜 · 逐帧精准"
        fontSize={24}
        position={{ x: W / 2, y: 355 }}
        align="center"
        start={0}
        duration={20}
      />
      <DrawShape
        type="rounded-rectangle"
        position={{ x: 260, y: 400 }}
        size={{ width: 220, height: 36 }}
        start={0}
        duration={12}
        fillColor="rgba(52,152,219,0.12)"
        strokeColor={ACCENT}
      />
      <DrawText
        text="白板组件"
        fontSize={15}
        fontWeight="bold"
        position={{ x: 370, y: 420 }}
        align="center"
        start={0}
        duration={12}
      />
      <DrawShape
        type="rounded-rectangle"
        position={{ x: 510, y: 400 }}
        size={{ width: 220, height: 36 }}
        start={0}
        duration={12}
        fillColor="rgba(44,62,80,0.08)"
      />
      <DrawText
        text="打包与渲染"
        fontSize={15}
        fontWeight="bold"
        position={{ x: 620, y: 420 }}
        align="center"
        start={0}
        duration={12}
      />
      <DrawShape
        type="rounded-rectangle"
        position={{ x: 760, y: 400 }}
        size={{ width: 220, height: 36 }}
        start={0}
        duration={12}
        fillColor="rgba(44,62,80,0.08)"
      />
      <DrawText
        text="手绘白板风格"
        fontSize={15}
        fontWeight="bold"
        position={{ x: 870, y: 420 }}
        align="center"
        start={0}
        duration={14}
      />
      <Hand action="write" follow={true} visible={true} />
    </WhiteboardScene>
  );
}

function CtaScene() {
  return (
    <WhiteboardScene width={W} height={H} texture="whiteboard" theme={excalidrawTheme}>
      <DrawShape
        type="rounded-rectangle"
        position={{ x: 300, y: 260 }}
        size={{ width: 680, height: 200 }}
        start={0}
        duration={20}
        strokeColor={CTA}
        fillColor="none"
      />
      <DrawText
        text="立即开始构建"
        fontSize={44}
        fontWeight="bold"
        position={{ x: W / 2, y: 340 }}
        align="center"
        start={0}
        duration={24}
        strokeColor={CTA}
      />
      <DrawText
        text="开源免费 · 手写分镜生成讲解视频"
        fontSize={20}
        position={{ x: W / 2, y: 400 }}
        align="center"
        start={0}
        duration={20}
      />
      <Hand action="write" follow={true} visible={true} />
    </WhiteboardScene>
  );
}

export default function SeqvioIntroZh() {
  return (
    <VideoComposition
      id="seqvio-intro-zh"
      width={W}
      height={H}
      fps={30}
      duration={660}
      backgroundColor="#ffffff"
    >
      <Scene id="title" duration={200}>
        <TitleScene />
      </Scene>
      <Transition type="fade" duration={12} />
      <Scene id="pipeline" duration={220}>
        <PipelineScene />
      </Scene>
      <Transition type="fade" duration={12} />
      <Scene id="features" duration={142}>
        <FeatureScene />
      </Scene>
      <Transition type="fade" duration={12} />
      <Scene id="cta" duration={62}>
        <CtaScene />
      </Scene>
    </VideoComposition>
  );
}

export const meta = {
  duration: 660,
  fps: 30,
};
