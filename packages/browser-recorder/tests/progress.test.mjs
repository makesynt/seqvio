...
import { browserRenderProgressPercent } from '../dist/pipeline.js';

test('browser render phase mapping is monotonic', () => {
  const points = [
  ...
  ].map((progress) => browserRenderProgressPercent({ ...progress, message: '' }));
  assert.deepEqual(points, [...points].sort((a, b) => a - b));
  assert.equal(points[0], 65);
