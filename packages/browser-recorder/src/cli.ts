...
function packageVersion(): string {
  ...
  return (
    JSON.parse(fs.readFileSync(packagePath, "utf8")) as { version: string }
  ).version;
}
...
export async function runBrowserCli(
  ...
  try {
    ...
    const failure = captureCliFailure({
      adapter: "browser",
      exitCode: CaptureCliExitCode.usage,
    ...
    try {
      if (
        [
          ...
          "voice",
          "qaConfig",
        ].some((name) => parsed.values.has(name))
      ) {
        throw new CliUsageError(
          "recording, audio, and QA options are only valid with record",
        );
      }
  ...
  try {
    ...
    if ((audioProvider || audioVoice) && !withAudio)
      throw new Error("--provider and --voice require --withAudio");
    if (parsed.values.has("burnCaptions") && !withAudio)
      throw new Error("--burnCaptions requires --withAudio");
    jobId = validateCaptureJobId(
      stringValue(parsed.values, "jobId") ??
        createCaptureJobId(dependencies.now?.()),
    );
    const candidateJobDir = path.join(outputRoot(parsed.values), jobId);
    if (
      fs.existsSync(candidateJobDir) &&
      fs.readdirSync(candidateJobDir).length > 0
    ) {
      throw new Error(
        `Job directory already exists and is not empty: ${candidateJobDir}`,
      );
    }
    ...
    const result: BrowserPipelineResult = await (
      dependencies.pipeline ?? runPipeline
    ...
    const exitCode = pipelineStarted
      ? CaptureCliExitCode.pipeline
      : CaptureCliExitCode.usage;
    ...
    const failure = captureCliFailure({
      ...
      exitCode,
      error: {
        code: pipelineStarted ? "pipeline_failed" : "invalid_input",
        message,
      },
      jobId,
      jobDir,
...
if (require.main === module) {
  runBrowserCli(process.argv.slice(2)).then(
    ...
    (error) => {
      ...
      process.exitCode = CaptureCliExitCode.internal;
    },
  );
}
