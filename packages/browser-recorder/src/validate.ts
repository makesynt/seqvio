...
import { normalizePrivacyPolicy } from "./privacy";
...
function validateAction(action: BrowserAction, index: number): void {
  ...
  if (action.type === "navigate") {
    assert(
      ...
      `${action.id}.value is required`,
    );
  }
  if (action.type === "press") {
    assert(
      typeof action.key === "string" && action.key.length > 0,
      `${action.id}.key is required`,
    );
  }
}
...
export function validatePlan(value: unknown): BrowserRecordingPlan {
  ...
  assert(
    ...
    "startUrl must use http, https, or file",
  );
  assert(
    plan.viewport && Number.isFinite(plan.viewport.width),
    "viewport.width is required",
  );
  assert(
    plan.viewport && Number.isFinite(plan.viewport.height),
    "viewport.height is required",
  );
  assert(
    plan.viewport.width >= 640 && plan.viewport.width <= 3840,
    "viewport.width must be 640-3840",
  );
  assert(
    plan.viewport.height >= 360 && plan.viewport.height <= 2160,
    "viewport.height must be 360-2160",
  );
  assert(
  ...
  return {
    ...
    privacy: normalizePrivacyPolicy(plan.privacy),
  };
}
