...
} from "../narration-anchor";
import { validateStyleProfile } from "../style-profile";
import { isSoundCueName } from "../sound";
...
function validateInfographicScene(
  ...
  if (
    ...
    !["auto", "standard", "reduced"].includes(scene.density)
  ) {
    issue(issues, {
      severity: "error",
      path: `${scenePath}.density`,
      code: "unsupported_infographic_density",
      message: `${scenePath}.density must be auto, standard, or reduced`,
      repairable: true,
    });
  }
  const collections = [
    ...
    items.forEach((item, index) => {
      ...
      if (typeof item.id !== "string" || item.id.length === 0) {
        issue(issues, {
          severity: "error",
          path: `${itemPath}.id`,
          code: "missing_infographic_item_id",
          message: `${itemPath}.id must be a non-empty string`,
          repairable: true,
        });
        ...
        issue(issues, {
          severity: "error",
          path: `${itemPath}.id`,
          code: "duplicate_infographic_item_id",
          message: `Duplicate infographic item id "${item.id}"`,
          repairable: true,
        });
      ...
      if (
        ...
        issue(issues, {
          severity: "error",
          path: `${itemPath}.label`,
          code: "missing_infographic_item_label",
          message: `${itemPath}.label must be a non-empty string`,
          repairable: true,
        });
      }
      if (
        ...
        issue(issues, {
          severity: "error",
          path: `${itemPath}.title`,
          code: "missing_infographic_chart_title",
          message: `${itemPath}.title must be a non-empty string`,
          repairable: true,
        });
      }
      if (
        "at" in item &&
        item.at !== undefined &&
        (typeof item.at !== "number" || item.at < 0)
      ) {
        issue(issues, {
          severity: "error",
          path: `${itemPath}.at`,
          code: "invalid_infographic_item_time",
          message: `${itemPath}.at must be a non-negative number`,
          repairable: true,
        });
      }
    });
  ...
  const nodeIds = new Set(
    (scene.relationshipNodes ?? []).map((node) => node.id),
  );
  (scene.relationships ?? []).forEach((relationship, index) => {
    if (!nodeIds.has(relationship.from))
      issue(issues, {
        severity: "error",
        path: `${scenePath}.relationships[${index}].from`,
        code: "invalid_infographic_relationship_from",
        message: "Relationship source must reference a relationship node",
        repairable: true,
      });
    if (!nodeIds.has(relationship.to))
      issue(issues, {
        severity: "error",
        path: `${scenePath}.relationships[${index}].to`,
        code: "invalid_infographic_relationship_to",
        message: "Relationship target must reference a relationship node",
        repairable: true,
      });
  });
  (scene.charts ?? []).forEach((chart, chartIndex) => {
    ...
    if (chart.kind !== "bar" && chart.kind !== "line")
      issue(issues, {
        severity: "error",
        path: `${chartPath}.kind`,
        code: "invalid_infographic_chart_kind",
        message: "Chart kind must be bar or line",
        repairable: true,
      });
    if (!Array.isArray(chart.series) || chart.series.length === 0) {
      issue(issues, {
        severity: "error",
        path: `${chartPath}.series`,
        code: "missing_infographic_chart_series",
        message: "Chart must contain at least one data series",
        repairable: true,
      });
    }
    ...
    (chart.series ?? []).forEach((series, seriesIndex) => {
      ...
      if (!series.id || seriesIds.has(series.id))
        issue(issues, {
          severity: "error",
          path: `${seriesPath}.id`,
          code: series.id
            ? "duplicate_infographic_series_id"
            : "missing_infographic_series_id",
          message:
            "Chart series ids must be non-empty and unique within the chart",
          repairable: true,
        });
      ...
      if (!series.label)
        issue(issues, {
          ...
          code: "missing_infographic_series_label",
          message: "Chart series label is required",
          repairable: true,
        });
      if (
        ...
        series.points.some(
          (point) => typeof point.x !== "string" || !Number.isFinite(point.y),
        )
      ) {
        issue(issues, {
          severity: "error",
          path: `${seriesPath}.points`,
          code: "invalid_infographic_chart_points",
          message:
            "Chart points require a string x value and finite numeric y value",
          repairable: true,
        });
      }
    });
    if (
      ...
      issue(issues, {
        severity: "error",
        path: `${chartPath}.yAxis`,
        code: "invalid_infographic_axis_domain",
        message: "yAxis min must be less than max",
        repairable: true,
      });
    }
  });
...
function validatePlaceholderScene(
  ...
  if (scene.type === "terminal") {
    ...
    const hasEvents =
      Array.isArray(terminal.events) && terminal.events.length > 0;
    const hasCommands =
      Array.isArray(terminal.commands) && terminal.commands.length > 0;
...
function validateManimScene(
  ...
  if (typeof scene.sourceVideo !== "string" || !scene.sourceVideo.trim()) {
    issue(issues, {
      severity: "error",
      path: `${scenePath}.sourceVideo`,
      code: "missing_manim_media",
      message: `${scenePath}.sourceVideo must reference rendered media`,
      repairable: true,
    });
  }
  const markerIds = new Set<string>();
  const beatIds = new Set(
    (scene.explanation?.beats ?? []).map((beat) => beat.id),
  );
  (scene.markers ?? []).forEach((marker, index) => {
    ...
    if (!marker.id)
      ...
      issue(issues, {
        severity: "error",
        path: `${path}.id`,
        code: "duplicate_manim_marker_id",
        message: `Duplicate Manim marker id "${marker.id}"`,
        repairable: true,
      });
    ...
    if (!Number.isInteger(marker.frame) || marker.frame < 0)
      issue(issues, {
        severity: "error",
        path: `${path}.frame`,
        code: "invalid_manim_marker_frame",
        message: `${path}.frame must be a non-negative integer`,
        repairable: true,
      });
    if (marker.beatId && !beatIds.has(marker.beatId))
      issue(issues, {
        severity: "error",
        path: `${path}.beatId`,
        code: "unknown_manim_marker_beat",
        message: `${path}.beatId must reference an ExplanationBeat in the same scene`,
        repairable: true,
      });
  });
}
...
function validateAnnotation(
  ...
  if (
    typeof annotation.targetId !== "string" ||
    annotation.targetId.length === 0
  ) {
    issue(issues, {
      severity: "error",
      path: `${path}.targetId`,
  ...
  if (
    annotation.kind === "connector" &&
    (typeof annotation.toTargetId !== "string" ||
      annotation.toTargetId.length === 0)
  ) {
    issue(issues, {
      severity: "error",
      path: `${path}.toTargetId`,
      code: "missing_connector_target",
      message: `${path}.toTargetId must be a non-empty string for connector annotations`,
      repairable: true,
    });
  }
  if (
    annotation.kind === "guided-path" &&
    (!Array.isArray(annotation.pathTargetIds) ||
      annotation.pathTargetIds.length < 2)
  ) {
    issue(issues, {
      severity: "error",
      path: `${path}.pathTargetIds`,
      code: "invalid_guided_path_targets",
      message: `${path}.pathTargetIds must contain at least two target ids`,
      repairable: true,
    });
  }
  if (
...
function collectAddressableIds(scene: SceneSpec): string[] {
  ...
  if (scene.type === "diagram") {
    ...
    for (const chart of scene.charts ?? [])
      for (const series of chart.series ?? []) ids.push(series.id);
  ...
  explanation.cues.forEach((cue, index) => {
    ...
    if (!isObject(cue) || typeof cue.id !== "string" || !cue.id.trim()) {
      issue(issues, {
        severity: "error",
        path: `${cuePath}.id`,
        code: "missing_explanation_cue_id",
        message: `${cuePath}.id must be a non-empty string`,
        repairable: true,
      });
      return;
    }
    if (cues.has(cue.id)) {
      issue(issues, {
        severity: "error",
        path: `${cuePath}.id`,
        code: "duplicate_explanation_cue_id",
        message: `Duplicate explanation cue id "${cue.id}"`,
        repairable: true,
      });
    }
    if (typeof cue.text !== "string" || !normalizeNarrationText(cue.text)) {
      issue(issues, {
        severity: "error",
        path: `${cuePath}.text`,
        code: "missing_explanation_cue_text",
        message: `${cuePath}.text must be a non-empty string`,
        repairable: true,
      });
    }
    cues.set(cue.id, {
  ...
  explanation.beats.forEach((beat, index) => {
    ...
    if (!isObject(beat) || typeof beat.id !== "string" || !beat.id.trim()) {
      issue(issues, {
        severity: "error",
        path: `${beatPath}.id`,
        code: "missing_explanation_beat_id",
        message: `${beatPath}.id must be a non-empty string`,
        repairable: true,
      });
      return;
    }
    if (beatIds.has(beat.id)) {
      issue(issues, {
        severity: "error",
        path: `${beatPath}.id`,
        code: "duplicate_explanation_beat_id",
        message: `Duplicate explanation beat id "${beat.id}"`,
        repairable: true,
      });
    }
    beatIds.add(beat.id);
    if (typeof beat.cueId !== "string" || !cues.has(beat.cueId)) {
      issue(issues, {
        severity: "error",
        path: `${beatPath}.cueId`,
        code: "unknown_explanation_cue",
        message: `${beatPath}.cueId must reference an explanation cue`,
        repairable: true,
      });
    }
    if (
      !isObject(beat.anchor) ||
      typeof beat.anchor.text !== "string" ||
      !normalizeNarrationText(beat.anchor.text)
    ) {
      issue(issues, {
        severity: "error",
        path: `${beatPath}.anchor`,
        code: "missing_beat_anchor",
        message: `${beatPath}.anchor.text must be a non-empty string`,
        repairable: true,
      });
      ...
      if (
        ...
        issue(issues, {
          severity: "error",
          path: `${beatPath}.anchor.occurrence`,
          code: "invalid_beat_anchor_occurrence",
          message: `${beatPath}.anchor.occurrence must be a positive integer`,
          repairable: true,
        });
      }
      const matches = findNarrationAnchorMatches(
        cues.get(beat.cueId)!.text,
        beat.anchor.text,
      );
      if (
        matches.length === 0 ||
        (occurrence !== undefined && matches[occurrence - 1] === undefined)
      ) {
        issue(issues, {
          severity: "error",
          path: `${beatPath}.anchor.text`,
          code: "missing_beat_anchor",
          message: `Anchor "${beat.anchor.text}" was not found in cue "${beat.cueId}"`,
          repairable: true,
        });
        ...
        issue(issues, {
          severity: "error",
          path: `${beatPath}.anchor`,
          code: "ambiguous_beat_anchor",
          message: `Anchor "${beat.anchor.text}" occurs ${matches.length} times; set occurrence`,
          repairable: true,
        });
      } else {
        ...
        if (previous !== undefined && anchorIndex < previous) {
          issue(issues, {
            severity: "error",
            path: `${beatPath}.anchor`,
            code: "non_monotonic_beat_anchor",
            message: `Beat anchor order reverses within cue "${beat.cueId}"`,
            repairable: true,
          });
        }
    ...
    if (!Array.isArray(beat.visuals) || beat.visuals.length === 0) {
      issue(issues, {
        severity: "error",
        path: `${beatPath}.visuals`,
        code: "missing_beat_visuals",
        message: `${beatPath}.visuals must be a non-empty array`,
        repairable: true,
      });
    } else {
      beat.visuals.forEach((visual, visualIndex) => {
        ...
        if (
          !isObject(visual) ||
          typeof visual.targetId !== "string" ||
          !addressableIds.has(visual.targetId)
        ) {
          issue(issues, {
            severity: "error",
            path: `${visualPath}.targetId`,
            code: "unknown_beat_visual_target",
            message: `${visualPath}.targetId must reference an addressable element in the scene`,
            repairable: true,
          });
        }
        if (
          ...
          ![
            ...
            "emphasize",
            "transform",
          ].includes(String(visual.action))
        ) {
          issue(issues, {
            severity: "error",
            path: `${visualPath}.action`,
            code: "unsupported_beat_visual_action",
            message: `${visualPath}.action is unsupported`,
            repairable: true,
          });
        }
        if (
          ...
          (typeof visual.relatedTargetId !== "string" ||
            !addressableIds.has(visual.relatedTargetId))
        ) {
          issue(issues, {
            severity: "error",
            path: `${visualPath}.relatedTargetId`,
            code: "invalid_compare_visual_target",
            message: `${visualPath}.relatedTargetId must reference an addressable element`,
            repairable: true,
          });
        }
        if (
          isObject(visual) &&
          visual.action === "trace" &&
          (!Array.isArray(visual.pathTargetIds) ||
            visual.pathTargetIds.length < 2 ||
            visual.pathTargetIds.some(
              (id) => typeof id !== "string" || !addressableIds.has(id),
            ))
        ) {
          issue(issues, {
            severity: "error",
            path: `${visualPath}.pathTargetIds`,
            code: "invalid_trace_visual_path",
            message: `${visualPath}.pathTargetIds must reference at least two addressable elements`,
            repairable: true,
          });
        }
        if (
          isObject(visual) &&
          visual.offsetMs !== undefined &&
          !Number.isFinite(visual.offsetMs)
        ) {
          issue(issues, {
            severity: "error",
            path: `${visualPath}.offsetMs`,
            code: "invalid_beat_visual_offset",
            message: `${visualPath}.offsetMs must be finite`,
            repairable: true,
          });
        }
        if (
          isObject(visual) &&
          visual.minHoldMs !== undefined &&
          (!Number.isFinite(visual.minHoldMs) || Number(visual.minHoldMs) <= 0)
        ) {
          issue(issues, {
            severity: "error",
            path: `${visualPath}.minHoldMs`,
            code: "invalid_beat_visual_hold",
            message: `${visualPath}.minHoldMs must be greater than zero`,
            repairable: true,
          });
        }
      });
    }
    if (beat.sounds !== undefined) {
      if (!Array.isArray(beat.sounds)) {
        issue(issues, {
          severity: "error",
          path: `${beatPath}.sounds`,
          code: "invalid_beat_sounds",
          message: `${beatPath}.sounds must be an array`,
          repairable: true,
        });
      } else {
        beat.sounds.forEach((sound, soundIndex) => {
          const soundPath = `${beatPath}.sounds[${soundIndex}]`;
          if (!isObject(sound) || !isSoundCueName(sound.cue)) {
            issue(issues, {
              severity: "error",
              path: `${soundPath}.cue`,
              code: "unsupported_sound_cue",
              message: `${soundPath}.cue must be a supported local SoundCue`,
              repairable: true,
            });
          }
          if (
            isObject(sound) &&
            sound.intensity !== undefined &&
            (typeof sound.intensity !== "number" ||
              sound.intensity < 0 ||
              sound.intensity > 1)
          ) {
            issue(issues, {
              severity: "error",
              path: `${soundPath}.intensity`,
              code: "invalid_sound_intensity",
              message: `${soundPath}.intensity must be between 0 and 1`,
              repairable: true,
            });
          }
          if (
            isObject(sound) &&
            sound.repeat !== undefined &&
            (typeof sound.repeat !== "number" ||
              !Number.isInteger(sound.repeat) ||
              sound.repeat < 1)
          ) {
            issue(issues, {
              severity: "error",
              path: `${soundPath}.repeat`,
              code: "invalid_sound_repeat",
              message: `${soundPath}.repeat must be a positive integer`,
              repairable: true,
            });
          }
        });
    ...
    const captureStepId = isObject(beat.evidence)
      ? beat.evidence.captureStepId
      : undefined;
    if (
      captureStepId !== undefined &&
      (typeof captureStepId !== "string" || !captureStepIds.has(captureStepId))
    ) {
      issue(issues, {
        severity: "error",
        path: `${beatPath}.evidence.captureStepId`,
        code: "unknown_beat_capture_step",
        message: `${beatPath}.evidence.captureStepId must reference a capture step in this scene`,
        repairable: true,
      });
    }
  });
...
export function validateExplainerDocument(input: unknown): CompositionIssue[] {
    ...
    return [
      {
        ...
        repairable: true,
        suggestion:
          'Return one JSON object with format "seqvio-explainer", schemaVersion "1.0", id, and scenes.',
      },
    ];
  }
  ...
  if (doc.schemaVersion !== EXPLAINER_DOCUMENT_SCHEMA_VERSION) {
    issue(issues, {
      ...
      repairable: true,
      suggestion:
        'Set schemaVersion to "1.0". This field is an implementation compatibility marker, not the product name.',
    });
  }
  ...
  if (
    doc.pacingProfile !== undefined &&
    !isPacingProfileId(doc.pacingProfile)
  ) {
    issue(issues, {
      severity: "error",
      path: "pacingProfile",
  ...
  doc.scenes.forEach((scene, sceneIndex) => {
    ...
    if (typedScene.explanation !== undefined) {
      if (!isObject(typedScene.explanation)) {
        issue(issues, {
          severity: "error",
          path: `${scenePath}.explanation`,
          code: "invalid_scene_explanation",
          message: `${scenePath}.explanation must be an object`,
          repairable: true,
        });
      } else {
        validateExplanation(
          typedScene.explanation,
          typedScene,
          scenePath,
          issues,
        );
      ...
      if (isObject(annotation) && Array.isArray(annotation.pathTargetIds)) {
        annotation.pathTargetIds.forEach((targetId, targetIndex) => {
          if (typeof targetId === "string")
            annotationTargets.push({
              path: `annotations[${index}].pathTargetIds[${targetIndex}]`,
              targetId,
            });
        });
      }
    ...
    scene.annotations.forEach((annotation, annotationIndex) => {
      ...
      if (isObject(annotation) && Array.isArray(annotation.pathTargetIds)) {
        annotation.pathTargetIds.forEach((targetId, targetIndex) => {
          if (typeof targetId === "string")
            annotationTargets.push({
              path: `scenes[${sceneIndex}].annotations[${annotationIndex}].pathTargetIds[${targetIndex}]`,
              targetId,
            });
        });
      }
