...
import { diagnoseResolvedSoundTracks } from "./sound";
...
export function reflowSynthesizedTimeline(
  ...
  const sceneTimings: NonNullable<CompositionAudioManifest["sceneTimings"]> =
    [];
  const explanationBeats: NonNullable<
    CompositionAudioManifest["explanationBeats"]
  > = [];
  let frameCursor = 0;
  ...
  for (const scene of manifest.sceneTimings) {
    ...
    const sceneCues = synthesizedNarration.filter(
    ...
    const narrationFrames = Math.ceil(
      ((cueCursorMs - sceneStartMs + (sceneCues.length ? sceneTailMs : 0)) /
        1000) *
        fps,
    );
    ...
    const resolvedSceneBeats = (manifest.explanationBeats ?? [])
      ...
      .map((beat) => {
        ...
        if (!resolution.ok)
          return { ...beat, resolutionError: resolution.code };
        return {
          ...beat,
    ...
    const semanticPoints = resolvedSceneBeats
      .filter(
        (beat): beat is typeof beat & { outputFrame: number } =>
    ...
    const orderedHighlights = [...(scene.highlights ?? [])].sort(
    ...
    const chunkFrames = sceneCues
      .flatMap((cue) => {
        ...
        const cueStartFrame =
          resolvedCue.startFrame ??
          Math.round(
            (resolveNarrationCueTimes(resolvedCue, fps).startMs / 1000) * fps,
          );
        const cueSceneOffset = Math.max(0, cueStartFrame - frameCursor);
        return (cue.chunks ?? []).map(
          (chunk) => cueSceneOffset + chunk.offsetFrame,
        );
      })
    ...
    const fallbackPoints = Array.from({ length: mappedCount }, (_, index) => ({
      ...
      sourceFrame: Math.min(
        ...
        orderedHighlights[index].startFrame,
      ),
    }));
    const anchorPoints =
      semanticPoints.length > 0 ? semanticPoints : fallbackPoints;
    const timeMap =
    ...
    frameCursor +=
      durationFrames + Math.max(0, scene.transitionAfterFrames ?? 0);
  }

  return {
    narration: synthesizedNarration.map(
      (cue) => resolvedById.get(cue.id) ?? cue,
    ),
    sceneTimings,
    explanationBeats,
...
export function buildManifestFromMeta(
  ...
  const hasAudio = Boolean(meta.audio) || Boolean(meta.captions?.length);
...
export function validateAudioManifest(
  ...
  if (
    manifest.pacingProfile !== undefined &&
    !isPacingProfileId(manifest.pacingProfile)
  ) {
    issues.push({
      severity: "error",
      ...
      repair:
        "Regenerate the manifest with a supported versioned pacing profile.",
    });
  }
  ...
  for (const [index, cue] of (manifest.narration ?? []).entries()) {
    ...
    if (!cue.id) {
      issues.push({
        severity: "error",
        code: "missing_narration_id",
        path: `${cuePath}.id`,
        message: 'Narration cue is missing "id".',
      });
    ...
    const authoredStart =
      cue.startMs ??
      (cue.startFrame !== undefined ? (cue.startFrame / fps) * 1000 : 0);
    const authoredEnd =
      cue.endMs ??
      (cue.endFrame !== undefined
        ? (cue.endFrame / fps) * 1000
        : authoredStart);
    if (authoredStart < 0 || authoredEnd < authoredStart) {
      issues.push({
        ...
        code:
          authoredStart < 0
            ? "negative_narration_time"
            : "invalid_narration_range",
        path: cuePath,
        message:
          authoredStart < 0
  ...
  for (const [index, cue] of (manifest.captions ?? []).entries()) {
    ...
    if (cue.startMs < 0 || cue.endMs <= cue.startMs) {
      issues.push({
        severity: "error",
        code:
          cue.startMs < 0 ? "negative_caption_time" : "invalid_caption_range",
        path: cuePath,
  ...
  for (const [index, scene] of (manifest.sceneTimings ?? []).entries()) {
    ...
    if (!scene.sceneId || seenSceneIds.has(scene.sceneId)) {
      issues.push({
        severity: "error",
        code: scene.sceneId
          ? "duplicate_audio_scene_timing"
          : "missing_audio_scene_id",
        path: `${scenePath}.sceneId`,
        message: scene.sceneId
          ? `Duplicate audio scene timing "${scene.sceneId}".`
          : "Audio scene timing is missing sceneId.",
      });
    }
      ...
      issues.push({
        ...
        code:
          scene.durationFrames <= 0
            ? "invalid_audio_scene_duration"
            : "non_monotonic_audio_scenes",
        path: scenePath,
        message:
          scene.durationFrames <= 0
            ? "Audio scene duration must be positive."
            : "Audio scene timings must be ordered by startFrame.",
      });
    }
    if (
      scene.sourceDurationFrames !== undefined &&
      scene.sourceDurationFrames <= 0
    ) {
      issues.push({
        severity: "error",
    ...
    for (
      let pointIndex = 1;
      pointIndex < (scene.timeMap?.length ?? 0);
      pointIndex++
    ) {
      ...
      if (
        ...
        issues.push({
          ...
          message:
            "Scene time-map anchors must increase in output time without reversing source time.",
        });
      }
    }
  ...
  const previousBeatByScene = new Map<
    string,
    { sourceFrame: number; outputFrame?: number }
  >();
  for (const [index, beat] of (manifest.explanationBeats ?? []).entries()) {
    ...
    if (!beat.id || seenBeatIds.has(beat.id)) {
      issues.push({
        severity: "error",
        code: beat.id
          ? "duplicate_explanation_beat_id"
          : "missing_explanation_beat_id",
        path: `${beatPath}.id`,
        message: beat.id
          ? `Duplicate explanation beat id "${beat.id}".`
          : "Explanation beat is missing id.",
      });
    }
    ...
    if (!cue) {
      issues.push({
        severity: "error",
        code: "unknown_explanation_beat_cue",
        path: `${beatPath}.cueId`,
    ...
    if (!seenSceneIds.has(beat.sceneId)) {
      issues.push({
        severity: "error",
        code: "unknown_explanation_beat_scene",
        path: `${beatPath}.sceneId`,
    ...
    if (!beat.anchor?.text?.trim()) {
      issues.push({
        severity: "error",
        code: "missing_explanation_beat_anchor",
    ...
    if (!Number.isFinite(beat.sourceFrame) || beat.sourceFrame < 0) {
      issues.push({
        severity: "error",
        code: "invalid_explanation_beat_source_frame",
        path: `${beatPath}.sourceFrame`,
        message: `Explanation beat "${beat.id}" has an invalid source frame.`,
      });
    }
    if (
      beat.outputFrame !== undefined &&
      (!Number.isFinite(beat.outputFrame) || beat.outputFrame < 0)
    ) {
      issues.push({
        severity: "error",
        code: "invalid_explanation_beat_output_frame",
        path: `${beatPath}.outputFrame`,
        message: `Explanation beat "${beat.id}" has an invalid resolved speech frame.`,
      });
    } else if (
      beat.resolutionError ||
      (beat.outputFrame === undefined && (cue?.chunks?.length ?? 0) > 0)
    ) {
      issues.push({
        severity: "error",
        code: "unresolved_explanation_beat_anchor",
        path: `${beatPath}.anchor`,
        ...
        repair:
          "Correct the anchor text or occurrence and synthesize narration again.",
      });
    }
    if (beat.confidence !== undefined && beat.confidence < 0.7) {
      issues.push({
        severity: "warning",
        code: "low_confidence_explanation_beat",
        path: beatPath,
        message: `Explanation beat "${beat.id}" resolved with confidence ${beat.confidence.toFixed(2)}.`,
        repair:
          "Use a more specific phrase anchor or a TTS provider with finer timing chunks.",
      });
    }
    ...
    if (
      ...
      (beat.sourceFrame < previous.sourceFrame ||
        (beat.outputFrame !== undefined &&
          previous.outputFrame !== undefined &&
          beat.outputFrame < previous.outputFrame))
    ) {
      issues.push({
        severity: "error",
        code: "non_monotonic_explanation_beats",
        path: beatPath,
  ...
  for (const [index, track] of (manifest.tracks ?? []).entries()) {
    ...
    if (!track.id) {
      issues.push({
        severity: "error",
        code: "missing_audio_track_id",
        path: `${trackPath}.id`,
        message: 'Audio track is missing "id".',
      });
    ...
    if (options.baseDir) {
      const resolvedTrack = resolveMaybeRelativePath(
        track.src,
        options.baseDir,
      );
  ...
  issues.push(...diagnoseResolvedSoundTracks(manifest));
